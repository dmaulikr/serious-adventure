Ext.define('Next.data.proxy.Mongo', function () {
    var _collectionName,
        _collection,
        _db;
    var _wrapSingleAction = function (actionName, currentAction) {
        var newAction = function () {
            var _args = arguments,
                me = this;
            if (_db.state !== 'connected' && _db.state !== 'connecting') {
	...
	return {
		...
		constructor: function (config) {
			...
			Next.data.proxy.Mongo.superclass.constructor.apply(this, arguments);
			try {
				mongo = require('mongodb');
			} catch (e) {
				console.error('Unable to find the node-mongodb-native adapter.' +
			...
			var Db = mongo.Db,
				...
				me = this;
			_db = new Db(dbName, new Server(host, port, {}), { safe: false, native_parser: true });
			this._count = 0;
			_wrapActions.call(this, ['read', 'update', 'destroy', 'create']);
		},
		alias: 'proxy.mongo',
		createObjectIDFromHexString: function (str) {
			try {
				return _db.bson_serializer.ObjectID.createFromHexString(str);
			} catch (e) {
				return undefined;
			}
		},
		...
		read: function (operation, callback, scope) {
			...
			_collection.find(
				operation.criteria || {},
				{ 'skip': operation.start, 'limit': operation.limit },
				function (err, cursor) {
